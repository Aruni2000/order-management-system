...
$countSql = "SELECT COUNT(*) as total FROM grn g LEFT JOIN suppliers s ON g.supplier_id = s.id WHERE g.status = 'cancelled'";
// Main query - cancelled GRNs only
$sql = "SELECT g.*, s.name as supplier_name FROM grn g LEFT JOIN suppliers s ON g.supplier_id = s.id WHERE g.status = 'cancelled'";
...
<head>
    <title>Cancelled GRNs | <?= htmlspecialchars($_SESSION['company_name'] ?? 'OMS') ?></title>
