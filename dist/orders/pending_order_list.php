                ...
                if (data.status === 'success') {
                    ...
                    if (data.tracking_numbers.length >= selectedCount) {
                        ...
                        data.tracking_numbers.slice(0, selectedCount).forEach((trackingNumber, index) => {
                            trackingHtml +=
                                `<div class="tracking-item">${index + 1}. ${trackingNumber}</div>`;
                        });
                        trackingHtml += `</div>`;
                        trackingHtml +=
                        ...
                        trackingHtml +=
                            `<div class="alert alert-warning mt-2">Only ${data.tracking_numbers.length} tracking numbers available, but ${selectedCount} orders selected.</div>`;
                        submitBtn.disabled = true;
                    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (apiDispatchForm) {
            apiDispatchForm.addEventListener('submit', function(e) {
                ...
                const dispatchTypeElement = document.querySelector(
                    'input[name="api_dispatch_type"]:checked');
                ...
                // Confirm action
                const actionText = dispatchType === 'new' ? 'create new API parcels' :
                    'assign existing tracking numbers';
                if (!confirm(
                        `Are you sure you want to ${actionText} for ${selectedOrders.length} orders?`
                        )) {
                    return;
                }
                ...
                if (carrier == 12) {
                    ...
                    endpoint = dispatchType === 'existing' ?
                        'royalexpress_bulk_existing_parcel_api.php' : '';
                } else {
                    endpoint = ''; // fallback
                }
                ...
                fetch(endpoint, {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            alert(
                                `Successfully processed ${data.processed_count || selectedOrders.length} orders via API!`);
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        alert(
                            'An error occurred while processing orders via API. Please try again.');
                    })
                    .finally(() => {
                        // Reset button state
                        submitBtn.innerHTML =
