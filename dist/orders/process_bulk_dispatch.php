<?php
// IMPORTANT: No whitespace or BOM before this line!
...
try {
    // First, verify all orders exist and are in pending status, AND get tenant_id
    $order_ids_str = implode(',', array_map('intval', $order_ids));
    $verify_query = "SELECT order_id, status, tenant_id FROM order_header WHERE order_id IN ($order_ids_str)";
    ...
    // Check if all orders are in pending status and have same tenant_id
    $invalid_orders = [];
    $tenant_ids = [];
    ...
    // Verify all orders belong to the same tenant
    $unique_tenants = array_unique($tenant_ids);
    if (count($unique_tenants) > 1) {
        throw new Exception('All orders must belong to the same tenant');
    }
    $order_tenant_id = $unique_tenants[0];
    
    // Verify courier exists, is active, AND belongs to the same tenant - ALSO GET co_id
    ...
    // Get unused tracking numbers for the selected courier AND tenant - ORDER BY tracking_id ASC
    $tracking_query = "SELECT t.tracking_id 
                       FROM tracking t
                       INNER JOIN couriers c ON t.courier_id = c.courier_id
                       WHERE t.courier_id = ? 
                       AND t.status = 'unused'
                       AND c.tenant_id = ?
                       ORDER BY CAST(t.tracking_id AS UNSIGNED) ASC 
                       LIMIT ?";
    
    $tracking_stmt = $conn->prepare($tracking_query);
    $tracking_count = count($order_ids);
    $tracking_stmt->bind_param("iii", $courier_id, $order_tenant_id, $tracking_count);
    ...
    if (count($tracking_numbers) < count($order_ids)) {
        throw new Exception('Not enough tracking numbers available for ' . $courier_name . ' in tenant ' . $order_tenant_id);
    }
    
    // Update orders with dispatch information - NOW INCLUDING co_id
    ...
    // Mark tracking numbers as used - with tenant verification
    $update_tracking_query = "UPDATE tracking t
                              INNER JOIN couriers c ON t.courier_id = c.courier_id
                              SET t.status = 'used', 
                                  t.updated_at = NOW() 
                              WHERE t.tracking_id = ?
    ...
    foreach ($order_ids as $index => $order_id) {
        ...
        // Update order header - NOW WITH co_id
        $update_order_stmt->bind_param("iissi", $courier_id, $courier_co_id, $tracking_number, $dispatch_notes, $order_id);
        if (!$update_order_stmt->execute()) {
            throw new Exception('Failed to update order: ' . $order_id);
        }
        
        if ($update_order_stmt->affected_rows === 0) {
            throw new Exception('Order ' . $order_id . ' was already processed by another user or status changed');
        }
        ...
        // Mark tracking number as used - with tenant verification
        $update_tracking_stmt->bind_param("sii", $tracking_number, $courier_id, $order_tenant_id);
        ...
        if ($update_tracking_stmt->affected_rows === 0) {
            throw new Exception('Tracking number ' . $tracking_number . ' was already used by another process or tenant mismatch');
        }
        ...
        $log_details = "Bulk dispatched order({$order_id}) with system tracking({$tracking_number}), co_id({$courier_co_id}), courier ID({$courier_id}) for tenant({$order_tenant_id})";
    ...
    if (isset($courier_stmt)) {
        $courier_stmt->close();
    }
}
// IMPORTANT: No whitespace after this closing tag!
?>