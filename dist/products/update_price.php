...
if (!in_array($role_id, [1, 3], true)) {
    ...
    echo json_encode(['success' => false, 'message' => 'Access denied. Only administrators and store users can update prices.']);
    exit();
}
...
try {
    ...
    if (!isset($input['product_id']) || !isset($input['new_lkr_price'])) {
        echo json_encode(['success' => false, 'message' => 'Missing required parameters']);
        exit();
    }

    $product_id = (int)$input['product_id'];
    $new_lkr_price = (float)$input['new_lkr_price'];
    ...
    $old_price = (float)$product['lkr_price'];
    $new_price_str = number_format($new_lkr_price, 2);
    ...
    try {
        ...
        $details = "Updated Price for Product '{$product['name']}' " .
            "(from LKR " . number_format($old_price, 2) . " to LKR {$new_price_str})";
